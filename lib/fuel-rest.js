...
var FuelRest = function( Auth, restOptions ) {
	'use strict';

	if( Auth instanceof FuelAuth ) {

		// if Auth is already a fuel-auth instance, use that
		this.AuthClient = Auth;

	} else if( _.isPlainObject( Auth ) ) {

		// otherwise if it's an object, we'll try to initialize a new FuelAuth for use
		try {
			this.AuthClient = new FuelAuth( Auth );
		} catch( err ) {
			console.log( err );
			return;
		}
